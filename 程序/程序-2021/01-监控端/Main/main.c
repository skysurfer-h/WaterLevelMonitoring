...
/***************************************蜂鸣器******************************************************/
void buzzer_mode(uchar buz_mode_new){//	 0-关 1-报警 2-提醒  3-响一声   定时器： buzzer_count  25ms
	static uchar buz_mode;
	if(buz_mode_new == 0 || buz_mode_new == 1 || \
		 buz_mode_new == 2 || buz_mode_new == 3)
	{
		buz_mode = buz_mode_new;
	}
	if(buz_mode==0){			 //关
		buzzer=1;
		buzzer_step=1;
...
i=buffer_begin;
  if(RX_finish==1){
	   //重复校验
	   if(buffer[i]==buffer[i+3] && buffer[i]==buffer[i+6] && \
			  buffer[i+1]==buffer[i+4] && buffer[i+1]==buffer[i+7] && \
